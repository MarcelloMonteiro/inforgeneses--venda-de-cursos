...
class Marcas extends CI_Controller{
    ...
    public function core($marca_id = NULL){
        ...
        $this->form_validation->set_rules('marca_nome', 'Nome da escola', 'trim|required|min_length[5]|max_length[40]|callback_valida_nome_marca');
        ...
        if($this->form_validation->run()){
            ...
            if(! $marca = $this->core_model->get_by_id('marcas', array('marca_id'=> $marca_id))){
                $this->session->set_flashdata('erro', 'A escola não foi encontrada');
                redirect('restrita/marcas');
    ...
    public function delete($marca_id = NULL){
        ...
        if($this->core_model->get_by_id('marcas', array('marca_id' => $marca_id, 'marca_ativa' => 1))){
            $this->session->set_flashdata('erro', 'Não é possivel excluir uma escola ativa');
                redirect('restrita/marcas');
        }
